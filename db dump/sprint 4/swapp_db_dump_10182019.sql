...
-- Dumping data for table swapp.account: ~106 rows (approximately)
...
-- Dumping data for table swapp.checklistactivitydate: ~35 rows (approximately)
...
-- Dumping data for table swapp.location: ~20 rows (approximately)
...
-- Dumping data for table swapp.narrative: ~7 rows (approximately)
...
-- Dumping data for table swapp.record: ~19 rows (approximately)
...
-- Dumping data for table swapp.recordaction: ~38 rows (approximately)
...
-- Dumping data for table swapp.recordphoto: ~40 rows (approximately)
...
-- Dumping data for table swapp.school: ~9 rows (approximately)
...
BEGIN
	...
	SELECT MAX(id) INTO v_id FROM checklistactivitydate;
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_checklistactivitydate_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_dashboard_gethazards`(
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_data_delete_aftersendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_hazard_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_location_getsendtoserver`()
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_narrative_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_recordaction_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_recordphoto_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_record_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_record_getsendtoserver_hazard`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_record_getsendtoserver_photo`(IN `i_cadId` INT)
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_sublocation_getsendtoserver`()
BEGIN
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_summary_getsendtoserver`(IN `i_cadId` INT)
BEGIN
...
-- Dumping data for table swapp.sublocation: ~19 rows (approximately)
...
-- Dumping data for table swapp.summary: ~13 rows (approximately)
...
-- Dumping data for table swapp.swt: ~0 rows (approximately)
...
-- Dumping data for table swapp.swtpermission: ~2 rows (approximately)
...
-- Dumping data for table swapp.user: ~56 rows (approximately)
...
-- Dumping data for table swapp.user_securityquestion: ~87 rows (approximately)

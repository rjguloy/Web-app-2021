...
-- Dumping data for table swapp.account: ~129 rows (approximately)
DELETE FROM `account`;
...
-- Dumping data for table swapp.location: ~139 rows (approximately)
...
-- Dumping data for table swapp.narrative: ~2 rows (approximately)
...
-- Dumping data for table swapp.record: ~2 rows (approximately)
DELETE FROM `record`;
...
-- Dumping data for table swapp.recordphoto: ~0 rows (approximately)
...
-- Dumping data for table swapp.school: ~89 rows (approximately)
DELETE FROM `school`;
/*!40000 ALTER TABLE `school` DISABLE KEYS */;
INSERT INTO `school` (`seqid`, `id`, `depedserver`, `name`, `address`, `approver`, `reviewer`, `createdby`, `createstatus`, `createdate`) VALUES
	(1, 100001, '0.0.0.0', 'Default School', NULL, 'Mr. Approver', 'Mr. Reviewer', 'SYSTEM', 'ACTIVE', '2020-04-18 04:24:10');
...
BEGIN
	...
	WHERE a.cad_id = i_cad
	AND a.createstatus = 'ACTIVE'
	AND a.validationdate IS NOT NULL 
	GROUP BY b.type
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_summary_forpdf`(
	IN `i_cad` INT
)
BEGIN
...
-- Dumping data for table swapp.sublocation: ~189 rows (approximately)
...
-- Dumping data for table swapp.summary: ~144 rows (approximately)
DELETE FROM `summary`;
...
-- Dumping data for table swapp.swtpermission: ~62 rows (approximately)
...
-- Dumping data for table swapp.user: ~76 rows (approximately)
DELETE FROM `user`;
...
-- Dumping data for table swapp.user_securityquestion: ~108 rows (approximately)

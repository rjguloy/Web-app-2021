...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_summary_hazardstatus_get_old`(
	IN `i_ids` VARCHAR(50)
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_summary_hazardtype_get_old`(
	IN `i_ids` VARCHAR(50)
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_swt_permissions_id_check`(
	IN `i_loc` INT
...
DELIMITER //
CREATE DEFINER=`root`@`%` PROCEDURE `sp_swt_permissions_update`(
	IN `i_team` ENUM('A','B','C'),
	IN `i_loc` INT

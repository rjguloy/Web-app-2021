...
-- Dumping data for table swapp.account: ~114 rows (approximately)
DELETE FROM `account`;
...
-- Dumping data for table swapp.checklistactivitydate: ~1 rows (approximately)
...
-- Dumping data for table swapp.location: ~3 rows (approximately)
...
-- Dumping data for table swapp.narrative: ~0 rows (approximately)
...
-- Dumping data for table swapp.recordaction: ~6 rows (approximately)
...
-- Dumping data for table swapp.school: ~3 rows (approximately)
DELETE FROM `school`;
/*!40000 ALTER TABLE `school` DISABLE KEYS */;
INSERT INTO `school` (`seqid`, `id`, `depedserver`, `name`, `address`, `approver`, `reviewer`, `createdby`, `createstatus`, `createdate`) VALUES
	(1, 100002, NULL, 'Default School Name', NULL, 'Mr. Approver', 'Mr. Reviewer', 'SYSTEM', 'ACTIVE', '2020-04-18 04:24:10');
...
BEGIN
	SELECT COUNT(a.hazardtype_id) AS type_count, b.name AS hazardtype_name
	FROM summary AS a LEFT JOIN hazardtype AS b 
	ON b.id = a.hazardtype_id 
	WHERE a.cad_id = i_cad AND a.createstatus = 'ACTIVE' 
	GROUP BY a.hazardtype_id ORDER BY a.hazardtype_id ASC;
END//
DELIMITER ;
...
-- Dumping data for table swapp.sublocation: ~16 rows (approximately)
...
-- Dumping data for table swapp.summary: ~12 rows (approximately)
DELETE FROM `summary`;
...
-- Dumping data for table swapp.swt: ~39 rows (approximately)
DELETE FROM `swt`;
...
-- Dumping data for table swapp.swtpermission: ~11 rows (approximately)
...
-- Dumping data for table swapp.user: ~59 rows (approximately)
DELETE FROM `user`;
...
-- Dumping data for table swapp.user_securityquestion: ~96 rows (approximately)
...
INSERT INTO `user_securityquestion` (`seqid`, `username`, `securityquestion_id`, `answer`, `createdate`) VALUES
	(1, 'superadmin', 10, 'a', '2019-08-22 07:52:50'),
	(2, 'superadmin', 7, 'a', '2019-08-22 07:52:50'),
	(3, 'superadmin', 6, 'a', '2019-08-22 07:52:50');
